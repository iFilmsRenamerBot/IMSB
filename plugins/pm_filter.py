...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("⚠️ 𝖧ᴇʏ, {query.from_user.first_name} ! 𝖲ᴇᴀʀᴄʜ 𝖸ᴏᴜʀ 𝖮ᴡɴ 𝖥ɪʟᴇ, 𝖣ᴏɴ'ᴛ 𝖢ʟɪᴄᴋ 𝖮ᴛʜᴇʀ𝗌 𝖱ᴇ𝗌ᴜʟᴛ𝗌 😬", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("⚠️ 𝖧ᴇʏ, {query.from_user.first_name} ! 𝖸ᴏᴜ Aʀᴇ U𝗌ɪɴɢ Oɴᴇ Oғ Mʏ Oʟᴅ Mᴇ𝗌𝗌ᴀɢᴇ𝗌, Sᴇɴᴅ Tʜᴇ Rᴇǫᴜᴇ𝗌ᴛ Aɢᴀɪɴ ⚠️", show_alert=True)
        return
...
async def advantage_spoll_choker(bot, query):
    ...
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("⚠️ 𝖧ᴇʏ, {query.from_user.first_name} ! 𝖸ᴏᴜ Aʀᴇ U𝗌ɪɴɢ Oɴᴇ Oғ Mʏ Oʟᴅ Mᴇ𝗌𝗌ᴀɢᴇ𝗌, Sᴇɴᴅ Tʜᴇ Rᴇǫᴜᴇ𝗌ᴛ Aɢᴀɪɴ ⚠️", show_alert=True)
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('⚠️ 𝖧ᴇʏ, {query.from_user.first_name} ! 𝖳ʜɪ𝗌 𝖬ᴏᴠɪᴇ/𝖲ᴇʀɪᴇ𝗌 𝖭ᴏᴛ Aᴅᴅᴇᴅ Iɴ @iFilms_Studios Dᴀᴛᴀʙᴀ𝗌ᴇ ⚠️')
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if (st.status == "creator") or (str(userid) in ADMINS):
            ...
            await query.answer("🤒 Hey, {query.from_user.first_name}! You Need To Be Group Owner Or An Auth User To Do That! 🤒", show_alert=True)
        ...
        if chat_type == "private":
            ...
            if (st.status == "creator") or (str(userid) in ADMINS):
                ...
                await query.answer("⚠️ Hey, {query.from_user.first_name}! That's Not For You!! ⚠️", show_alert=True)
    ...
    if query.data.startswith("file"):
        ...
        files_ = await get_file_details(file_id)
        if not files_:
            return await query.answer('Hello, {query.from_user.first_name}! No Such File Exist. Send Request Again')
        files = files_[0]
        title = files.file_name
        size = get_size(files.file_size)
        f_caption = files.caption
...
async def advantage_spell_chok(msg):
    ...
    gs_parsed = []
    if not g_s:
        if msg.chat.type not in ['group', 'supergroup'] or msg.chat.id in filters.chat(chats=MYGRP):
