...
def a(client, message: Message):
    ...
    for i in message.command[1:]:
        query += ' ' + str(i)
    print(query)
    m = message.reply(f"**🎧 Fɪɴᴅɪɴɢ Yᴏᴜʀ Sᴏɴɢ....PLᴢ Wᴀɪᴛ 🎸** `{urlissed}`", reply_to_message_id=reply_id)
    ...
    try:
        ...
        rep = f'🏷 <b>Tɪᴛʟᴇ :-</b> <a href="{link}">{title}</a>'
        ...
        m.edit('**An Error Occured. Please Report This To @Filmokamella !!**')
        print(e)
    try:
...
async def vsong(client, message: Message):
    ...
    pablo = await client.send_message(
        message.chat.id, f"**🎧 Sᴇᴀʀᴄʜɪɴɢ Yᴏᴜʀ Vɪᴅᴇᴏ....PLᴢ Wᴀɪᴛ 🎸** `{urlissed}`", reply_to_message_id=reply_id
    )
    if not urlissed:
